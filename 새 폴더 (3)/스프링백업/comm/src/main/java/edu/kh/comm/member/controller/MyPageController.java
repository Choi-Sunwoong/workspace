...
public class MyPageController {
	...
	public String changePW() {
		return "member/myPage-chagePw";
	}
	// 회원 탈퇴
	@GetMapping("/secession")
	public String secession(Member vo, HttpSession session, RedirectAttributes rttr) throws Exception {
	...
	public String updateInfo(@ModelAttribute("loginMember") Member loginMember,
							@RequestParam Map<String, Object> ParamMap, // 요청시 전달된 파라미터를 구분하지 않고 모두 Map에 담아둠
		...
		// 		--> @ModelAttriubte, @SessionAttributes 필요
		
		// @SessionAttriubutes 의 역할 2가지
		// 1) Model에 세팅 데이터의 key값을 @SessionAttriubtes에 작성하면 
		...
		//		단, @SessionAttriubtes({loginMember}) 가 클래스 위에 작성되어 있어야 가능
		
		// *** 매개변수를 이용해서 session, 파라미터 데이터를 동시에 얻어올 때 문제점 ***
		
		// sesssion에서 객체를 얻어오기 위해 매개변수에 작성한 상태
		// -> @ModelAttribute("loginMember") Member loginMember
