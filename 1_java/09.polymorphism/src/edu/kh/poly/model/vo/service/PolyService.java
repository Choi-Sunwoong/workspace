package edu.kh.poly.model.vo.service;
...
public class PolyService {
	...
	public void ex1() {
		...
		//The method setDiscountOffer(doulbe) is undefined for the type Car
		...
		// 상속 + 다형성
		// 상속 특징: 일련의 클래스에 대한 공통적인 규약 정의
		// 		-> Car 상속 클래스는 모두 gerEngine()을 가지고 있다를 정의.
	...
	public void ex2() {
		...
		// arr[1]이 참조하는 객체가 Sprak면 true, 아니면 false
		...
		for(int i = 0; i < arr.length; i++) {
			...
			if(arr[i] instanceof Tesla) { // 자식타입부터 먼저 검사 부모타입부터 검사하면 무조건 true가 되어서 끝나기 때문
	...
	public void ex3() { 
		...
		// Telsa = Tesla 같은 자료형 연산 가능하니 같은 주소를 복사시켜버림. (얕은 복사)
	...
	public void ex4() {
		...
		// 문제점 1.
		// ex) Spark가 Tesla 참조함. 부모를 공유할 뿐 다른건 겹치지 않음.
	...
	// 전달받은 Car 또는 자식개체의 엔진, 연료, 바퀴 개수를 출력하는 메소드
