...
public class MemberService {
	...
	public int updateMember() {
		...
		sc.nextLine(); //입력 버퍼 정리용 int인 경우에는 sc.nextLine();를 넣어주는게 좋다.
		...
		//    1 반환 
		loginMember.setMemberName(inputName); //private는 외부의 값을 가져오지 못하기 때문에 get,set을 이용하여 가져온다.
		// 입력받은 inputName을
	...
	public void searchRegion() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			// 2) memeberArr[i] 요소가 null 경우 반복 종료
			if(memberArr[i] == null) break;
			
			// 3) memberArr[i] 요소에 저장된 지역(gerRegion())이
