...
public class StudentService {
	...
	// 학생 정보를 저장할 List를 생성 (객제 배열 Upgrade 버전)
	...
	// 						리스트 크기가 늘었다 줄었다 하기 떄문에 큰 의미가 없어용
	
	// ArrayList(용량) : 용량 만큼의 리스트 생성
	//					너무 큰 값을 작성하면 메모리를 많이 소모함.
	
	//List studentList = new ArrayList();
	List<Student> studentList = new ArrayList<Student>(); //(검색) 조회 효율적
	...
	public void ex() {
		...
		// * 매개변수 타입이 Object == 모든 객체를 매개변수로 전달 할 수 잇음
	...
	public String updateStudent() {
		...
		System.out.println("=====학생 정보 수정======");
		 ...
		 // 2) 입력된 숫자가 0보다 작은가? (음수 X)
			...
			// 입력 받받은 index번쨰에 새로운 학생 정보를 세팅 == 수정
			// 이떄, index번째에 있던 기존 학생 정보가 반환된다.
